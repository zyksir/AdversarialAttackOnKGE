...
def load_link_prediction_data(name, use_test_set=False, limit=None, attack=None):
    """
    ...
    elif name.lower() == 'umls':
        train_file = locate_file('data/umls/train.txt')
        val_file = locate_file('data/umls/valid.txt')
        test_file = locate_file('data/umls/test.txt')
    else:
        raise ValueError(f'Could not find \'{name}\' dataset')
    ...
    if attack is not None:
      fake_triples = list(pickle.load(open(os.path.join("data/umls/", attack), "rb")))
