...
class DistMult(Module):
    ...
    def forward(self, triples, nodes, perturb_r=False, pertubation=None):
        ...
        if perturb_r == False:
          ...
          s, p, o = nodes[s_index, :], self.relations[p_index, :] + pertubation, nodes[o_index, :]

        scores = (s * p * o).sum(dim=-1)
        r_gradient = (s * o).sum(dim=-1)
        if self.b_init:
            scores = scores + (self.sbias[s_index] + self.pbias[p_index] + self.obias[o_index])

        return scores, r_gradient
